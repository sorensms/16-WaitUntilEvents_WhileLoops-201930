...
def double_a_float():
    point=float(input('What is the floating point number?'))
    print(point*2)
    """
...
def print_an_integer_many_times():
    number=int(input('What is the positive integer?'))
...
import math
def input_it_all():
    float2=float(input('What is the floating number?'))
    integer=int(input('What is the positive integer?'))
    string=input('What is the string?')
    for k in range(integer):
        print(math.sqrt(float2))
    for k in range (integer):
        print(string)
    """
    What comes in: Nothing.
